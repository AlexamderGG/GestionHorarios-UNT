...
const validarCurso = (data, isUpdate = false) => {
  ...
  if (!isUpdate || codigo !== undefined) {
    if (!codigo || codigo.trim().length < 2)
      errores.push("codigo es requerido (min 2 caracteres)");
  }
  if (!isUpdate || nombre !== undefined) {
    if (!nombre || nombre.trim().length < 3)
      errores.push("nombre es requerido (min 3 caracteres)");
  }
  if (!isUpdate || creditos !== undefined) {
    if (
      creditos === undefined ||
      creditos < 1 ||
      !Number.isInteger(Number(creditos))
    )
      errores.push("creditos debe ser un entero >= 1");
  }
  if (!isUpdate || ciclo !== undefined) {
    if (
      ...
      ciclo < 1 ||
      ciclo > 10 ||
      !Number.isInteger(Number(ciclo))
    )
      errores.push("ciclo debe ser un entero entre 1 y 10");
  }
  if (!isUpdate || semestre !== undefined) {
    if (!semestre || !SEMESTRE_REGEX.test(String(semestre).trim()))
      errores.push(
        "semestre debe tener formato YYYY-1 o YYYY-2. Ejemplo: 2026-1",
      );
  }
  if (
    horas_aula !== undefined &&
    (!Number.isInteger(Number(horas_aula)) || Number(horas_aula) < 0)
  ) {
    errores.push("horas_aula debe ser un entero >= 0");
  }
  if (
    horas_lab !== undefined &&
    (!Number.isInteger(Number(horas_lab)) || Number(horas_lab) < 0)
  ) {
    errores.push("horas_lab debe ser un entero >= 0");
  }
  ...
  update: async (req, res) => {
    try {
      ...
      if (req.body.codigo) {
        ...
        if (existe)
          return error(res, "Ya existe otro curso con ese código", 409);
      }
