  ...
  if (loading) {
    return (
      <div className="animate-fade-in max-w-2xl">
        <div className="skeleton h-7 w-56 mb-6 dark:opacity-20" />
        <div className="card p-6 space-y-4 dark:bg-neutral-800 dark:border-neutral-700">
          {[...Array(5)].map((_, i) => (
            <div key={i}>
              <div className="skeleton h-4 w-24 mb-2 dark:opacity-20" />
  ...
  return (
    <div className="animate-fade-in max-w-2xl pb-10">
      <div className="mb-6">
        ...
        <p className="text-sm text-neutral-500 dark:text-neutral-400 mt-1">Ajustes generales de horarios, bloques y semestre activo</p>
      </div>

      {/* TARJETA 1: CONFIGURACIÓN GENERAL */}
      <form onSubmit={handleSubmitConfig} className="card p-6 space-y-5 mb-8 dark:bg-neutral-800 dark:border-neutral-700">
        <div>
          ...
          <input
            ...
            className="input w-full sm:w-48 dark:bg-neutral-900 dark:border-neutral-700 dark:text-white"
            placeholder="2026-1"
            pattern="^\d{4}-[12]$"
            required
          />
        ...
        <div className="flex items-center justify-between p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg border border-indigo-100 dark:border-indigo-800/50 mt-4">
          <div>
            <h4 className="font-bold text-indigo-900 dark:text-indigo-300">Modo de Planificación por Turnos</h4>
            <p className="text-xs text-indigo-700 dark:text-indigo-400 mt-1 max-w-sm">
        ...
        {error && (
          <div className="text-sm text-danger-600 dark:text-danger-400 bg-danger-50 dark:bg-danger-900/30 border border-danger-200 dark:border-danger-800/50 rounded-lg px-4 py-2.5 flex items-center gap-2">
            <AlertCircle className="w-4 h-4" />
            {error}
          </div>
        )}

        <div className="pt-4 border-t border-neutral-100 dark:border-neutral-700/50 flex items-center gap-3">
          <button type="submit" disabled={saving} className="btn-primary flex items-center gap-2">
            {saving ? <><RefreshCw className="w-4 h-4 animate-spin" /> Guardando...</> : <><Save className="w-4 h-4" /> Guardar Configuración</>}
          </button>
          {saved && (
            <span className="flex items-center gap-1.5 text-sm text-success-600 dark:text-success-400 animate-slide-down">
      ...
      {user?.role === 'admin' && (
        <form onSubmit={handlePasswordSubmit} className="card p-6 space-y-5 dark:bg-neutral-800 dark:border-neutral-700">
          <div className="mb-2 border-b border-neutral-100 dark:border-neutral-700/50 pb-4">
          ...
          <div className="space-y-4 max-w-md">
            <div>
              <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1.5">Contraseña Actual</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="input w-full pl-10 pr-10 dark:bg-neutral-900 dark:border-neutral-700 dark:text-white"
            ...
            <div>
              <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1.5">Nueva Contraseña</label>
              <div className="relative">
                <Key className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-400 dark:text-neutral-500" />
                <input
              ...
              <ul className="text-xs text-neutral-500 dark:text-neutral-400 mt-2 space-y-1 ml-1 list-disc list-inside">
                <li className={passForm.newPassword.length >= 12 ? 'text-success-600 dark:text-success-400' : ''}>Mínimo 12 caracteres</li>
                <li className={/[A-Z]/.test(passForm.newPassword) ? 'text-success-600 dark:text-success-400' : ''}>Al menos una letra mayúscula</li>
                <li className={/[a-z]/.test(passForm.newPassword) ? 'text-success-600 dark:text-success-400' : ''}>Al menos una letra minúscula</li>
                <li className={/\d/.test(passForm.newPassword) ? 'text-success-600 dark:text-success-400' : ''}>Al menos un número</li>
              </ul>
            </div>
          </div>

          {passStatus.error && (
            <div className="text-sm text-danger-600 dark:text-danger-400 bg-danger-50 dark:bg-danger-900/30 border border-danger-200 dark:border-danger-800/50 rounded-lg px-4 py-2.5 flex items-center gap-2 max-w-md">
