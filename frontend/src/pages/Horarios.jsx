  ...
  const handleGenerar = async () => {
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        setMensaje({
          tipo: "error",
          texto: res.data?.message || "Error al generar",
        });
      }
    } catch (err) {
      setMensaje({
    ...
    <div className="animate-fade-in">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-neutral-500 mt-1">
            {horarios.length} clases programadas · {dias.length} días ·{" "}
      ...
      <div className="card mb-6">
        ...
        >
          ...
          <ChevronDown
            className={`w-4 h-4 text-neutral-400 transition-transform ${showFilters ? "rotate-180" : ""} lg:hidden`}
          />
        </button>
                ...
                >
                  ...
                  {docentes.map((d) => (
                    <option key={d.id} value={d.id}>
                      {d.nombres} {d.apellidos} ({d.categoria})
                    </option>
                  ))}
                </select>
              </div>
                ...
                <thead>
                  <tr className="bg-neutral-50">
                    ...
                    {dias.map((dia) => (
                      <th
                        key={dia}
                        className="border-b border-r border-neutral-200 p-3 text-center text-xs font-semibold text-neutral-500 uppercase tracking-wider min-w-[180px] last:border-r-0"
                      >
                        {dia}
                      </th>
                    ))}
                    ...
                    {horariosDelDia
                      ...
                      .map((h) => (
                        <div key={h.id} className="p-3 flex items-start gap-3">
                          ...
                          <div
                            className={`flex-1 rounded-lg p-2.5 border-l-3 ${
  ...
  <div className="card">
    <div className="flex flex-col items-center justify-center py-16 px-4">
      ...
      <p className="text-sm text-neutral-500 text-center max-w-md">
        Use el botón &quot;Generar Horarios&quot; para crear la programación del
        semestre, o ajuste los filtros para buscar horarios existentes.
      </p>
    </div>
  </div>
);
