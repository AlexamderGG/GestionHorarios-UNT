...
const AdminDocentes = () => {
  ...
  const cargarDatos = useCallback(async () => {
    ...
    try {
      ...
      if (configData) {
        ...
        if (configData.semestre_activo)
          setSemestreActivo(configData.semestre_activo);
      }
    } catch (err) {
      console.error("Error cargando datos:", err);
      setMensaje({
        tipo: "error",
        texto: "Error al cargar los datos del servidor",
      });
    } finally {
      setLoading(false);
    }
  ...
  const departamentosDisponibles = useMemo(() => {
    return [
      ...new Set(cursos.map((c) => c.especialidad).filter(Boolean)),
    ].sort();
  }, [cursos]);
    ...
    try {
      ...
      if (tipo === "f03") {
        const [resLectiva, resNoLectiva] = await Promise.all([
          api
            .get(`/docente/${docente.id}/horario-lectivo`, {
              params: { semestre: semestreActivo },
            })
            .catch(() => ({ data: { data: [] } })),
          api
            .get(`/docente/${docente.id}/horario-no-lectivo`, {
              params: { semestre: semestreActivo },
            })
            .catch(() => ({ data: { data: [] } })),
        ]);
        ...
        generarPDF_F03({
          perfil: {
            nombreCompleto: `${docente.nombres || ''} ${docente.apellidos || ''}`.toUpperCase().trim(),
            dni: docente.dni || '',
            categoria: docente.categoria || '',
            escuela: docente.escuela || 'Ingeniería de Sistemas',
            modalidad: docente.modalidad || 'Tiempo Completo'
          },
          horariosLectivos: lectivos,
          horariosNoLectivos: noLectivos,
      ...
      const res = await api.get(`/carga/docente/${docente.id}`, {
        params: { semestre: semestreActivo },
      });
      ...
      const datos = {
        docenteNombre:
          data.docenteNombre || `${docente.nombres} ${docente.apellidos}`,
        docenteDni: data.docenteDni || docente.dni || "",
        modalidad: data.modalidad || docente.modalidad || "Tiempo Completo",
        tipo_nombramiento:
          data.tipo_nombramiento || docente.tipo_nombramiento || "",
        ...
        form: {
          ...
          preparacion_clases_detalle:
            cargaNoLectiva.preparacion_clases_detalle || "",
          tutoria_consejeria: cargaNoLectiva.tutoria_consejeria || 0,
          tutoria_consejeria_detalle:
            cargaNoLectiva.tutoria_consejeria_detalle || "",
          ...
          otras_actividades_detalle:
            cargaNoLectiva.otras_actividades_detalle || "",
        },
      };
      ...
      setMensaje({
        tipo: "error",
        texto: "Error al obtener datos del docente para generar el formato.",
      });
      console.error(err);
    } finally {
  ...
  const docentesFiltrados = docentes.filter((d) => {
    ...
    const matchDep =
      !filtroDepartamento || d.especialidad === filtroDepartamento;
    ...
    try {
      ...
      if (editando) {
        ...
        if (res.data?.success) {
          setMensaje({
            tipo: "exito",
            texto: "Docente actualizado correctamente",
          });
        } else {
          setMensaje({
            tipo: "error",
            texto: res.data?.message || "Error al actualizar",
          });
        }
      } else {
        ...
        if (res.data?.success) {
          ...
          setMensaje({
            tipo: "error",
            texto: res.data?.message || "Error al crear",
          });
        }
      }
      setModalOpen(false);
  ...
  const handleEliminar = async (id) => {
    if (
      !confirm("¿Eliminar este docente? Se realizará una eliminación lógica.")
    )
      return;
    try {
  ...
  const badgeColor = (tipo) => {
    if (tipo === "Nombrado")
      return "bg-success-50 dark:bg-success-900/30 text-success-700 dark:text-success-400 border-success-200 dark:border-success-800/50";
  ...
  const catColor = (cat) => {
    switch (cat) {
      case "Principal":
        return "bg-primary-50 dark:bg-primary-900/30 text-primary-700 dark:text-primary-400 border-primary-200 dark:border-primary-800/50";
      case "Asociado":
      ...
      default:
        return "bg-neutral-50 dark:bg-neutral-800/50 text-neutral-600 dark:text-neutral-400 border-neutral-200 dark:border-neutral-700";
    }
  };
  ...
  return (
    <div className="animate-fade-in">
        ...
        <div className="flex flex-wrap gap-3 items-end">
          ...
          <div>
            ...
            <select
              ...
              className="input w-40 dark:bg-neutral-900 dark:border-neutral-700 dark:text-white"
            >
          ...
          <div>
            ...
            <select
              ...
              className="input w-36 dark:bg-neutral-900 dark:border-neutral-700 dark:text-white"
            >
          ...
          <div>
            ...
            <select
              ...
              className="input w-52 dark:bg-neutral-900 dark:border-neutral-700 dark:text-white"
            >
          ...
          <button
            onClick={cargarDatos}
            className="btn-secondary flex items-center gap-2 dark:border-neutral-700 dark:text-neutral-300 dark:hover:bg-neutral-700"
          >
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-neutral-50 dark:bg-neutral-900/50 border-b border-neutral-200 dark:border-neutral-700">
                ...
                <th className="text-center p-3 text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase w-20">
                  Antig.
                </th>
                <th className="text-center p-3 text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase w-36">
                  Acciones
                </th>
              </tr>
            </thead>
            <tbody>
              {docentesFiltrados.map((d) => (
                ...
                >
                  <td className="p-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {d.semestre_contrato && (
                          <p className="text-2xs text-warning-600 dark:text-warning-400">
                            Contrato: {d.semestre_contrato}
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  <td className="p-3">
                    <span
                      className={`inline-flex items-center px-2 py-0.5 rounded text-2xs font-medium border ${catColor(d.categoria)}`}
                    >
                      {d.categoria}
                    </span>
                  </td>
                  <td className="p-3 space-y-1">
                    <span
                      className={`flex w-fit items-center px-2 py-0.5 rounded text-2xs font-medium border ${badgeColor(d.tipo_nombramiento)}`}
                    >
                      {d.tipo_nombramiento}
                    </span>
                    ...
                    <div className="flex items-center justify-center gap-1">
                      ...
                      >
                        <button
                          onClick={() =>
                            setDropdownAbierto(
                              dropdownAbierto === d.id ? null : d.id,
                            )
                          }
                        ...
                        {dropdownAbierto === d.id && (
                          <div className="absolute right-0 top-8 z-50 w-56 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg shadow-lg py-1 animate-fade-in">
                            <p className="px-3 py-1.5 text-2xs font-semibold text-neutral-400 dark:text-neutral-500 uppercase tracking-wider">
                              Formatos Oficiales
                            </p>
              ...
              {docentesFiltrados.length === 0 && (
                <tr>
                  <td
                    colSpan={8}
                    className="p-12 text-center text-neutral-400 dark:text-neutral-500"
                  >
      ...
      {modalOpen && (
        ...
        >
          <div
            className="card p-6 w-full max-w-2xl shadow-modal animate-scale-in max-h-[90vh] overflow-y-auto dark:bg-neutral-800 dark:border-neutral-700"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    name="semestre_contrato"
                    value={form.semestre_contrato}
                    onChange={handleChange}
                    className="input w-full dark:bg-neutral-900 dark:border-neutral-700 dark:text-white"
                    placeholder="2026-1 (opcional)"
                  />
                </div>
              </div>
