...
const AdminExcepciones = () => {
  ...
  const getBadgeEstado = (estado) => {
    switch (estado) {
      case 'Aprobado': return 'bg-success-50 dark:bg-success-900/30 text-success-700 dark:text-success-400 border-success-200 dark:border-success-800/50';
      case 'Rechazado': return 'bg-danger-50 dark:bg-danger-900/30 text-danger-700 dark:text-danger-400 border-danger-200 dark:border-danger-800/50';
      default: return 'bg-warning-50 dark:bg-warning-900/30 text-warning-700 dark:text-warning-400 border-warning-200 dark:border-warning-800/50';
    }
  };
  ...
  if (loading) {
    return (
      <div className="p-6 space-y-4 max-w-6xl animate-pulse">
        ...
        <div className="h-10 bg-neutral-200 dark:bg-neutral-800 rounded w-1/2 mb-4" />
  ...
  return (
    <div className="p-4 max-w-6xl animate-fade-in">
      ...
      {error && (
        <div className="mb-4 px-4 py-2.5 bg-danger-50 dark:bg-danger-900/30 text-danger-700 dark:text-danger-400 border border-danger-200 dark:border-danger-800/50 rounded-xl text-xs flex items-center gap-2">
          <AlertCircle className="w-4 h-4" /> {error}
        </div>
      )}

      {/* Selectores de pestañas */}
      <div className="flex border-b border-neutral-200 dark:border-neutral-700 mb-6 gap-2 text-sm">
        <button
          onClick={() => setPestañaActiva('Pendiente')}
          className={`pb-2.5 px-4 font-medium relative transition-all ${
            pestañaActiva === 'Pendiente' 
        ...
        >
          ...
          {conteos.pendientes > 0 && (
            <span className="ml-2 bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-400 text-3xs font-bold px-2 py-0.5 rounded-full">
              {conteos.pendientes}
            </span>
          )}
        </button>
        ...
        <button
          ...
          className={`pb-2.5 px-4 font-medium relative transition-all ${
            pestañaActiva === 'Revisadas' 
              ? 'text-neutral-800 dark:text-white font-bold border-b-2 border-neutral-800 dark:border-white' 
              : 'text-neutral-500 dark:text-neutral-400 hover:text-neutral-800 dark:hover:text-neutral-200'
          }`}
        >
          📋 Historial Revisadas
          {conteos.revisadas > 0 && (
            <span className="ml-2 bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400 text-3xs font-bold px-2 py-0.5 rounded-full">
              {conteos.revisadas}
            </span>
          )}
        </button>
      ...
      {excepcionesFiltradas.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl shadow-2xs text-neutral-400 dark:text-neutral-500">
          <Inbox className="w-12 h-12 mb-2 text-neutral-300 dark:text-neutral-600" />
            ...
            <table className="w-full border-collapse text-left text-xs">
              ...
              <tbody className="divide-y divide-neutral-100 dark:divide-neutral-700/50">
                {excepcionesFiltradas.map((ex) => (
                  <tr key={ex.id} className="hover:bg-neutral-50/40 dark:hover:bg-neutral-700/30 transition-colors">
                    ...
                    <td className="p-4 whitespace-nowrap">
                      <span className="font-medium text-neutral-800 dark:text-neutral-200">{ex.curso_codigo}</span><br />
                      <span className="text-neutral-400 dark:text-neutral-500 text-3xs">{ex.curso_nombre} ({ex.tipo})</span>
                    </td>

                    <td className="p-4 text-neutral-600 dark:text-neutral-400 italic max-w-xs break-words">
                      &quot;{ex.motivo}&quot;
                    </td>
                      ...
                      {ex.horarios_solicitados && ex.horarios_solicitados.length > 0 ? (
                        <div className="space-y-1.5">
                          {ex.horarios_solicitados.map((hor, idx) => (
                            <div key={hor.id} className="bg-neutral-50 dark:bg-neutral-900/50 border border-neutral-200 dark:border-neutral-700 px-2 py-1 rounded text-3xs text-neutral-700 dark:text-neutral-300">
                              <span className="font-bold text-primary-700 dark:text-primary-400">Opción {idx + 1}:</span> Ciclo {hor.ciclo} · {hor.dia} {hor.hora_inicio?.slice(0,5)} ({hor.docente_apellidos})
                            </div>
                          ))}
                        </div>
                      ) : (
                        <span className="text-neutral-400 dark:text-neutral-500 italic text-2xs">Evaluación y reubicación libre</span>
                      )}
                    </td>
                    ...
                    <td className="p-4 text-center whitespace-nowrap">
                      <div className="flex items-center justify-center gap-2">
                        {ex.estado === 'Pendiente' ? (
                          ...
                          <span className="text-3xs text-neutral-500 dark:text-neutral-400 font-medium flex items-center gap-1 bg-neutral-50 dark:bg-neutral-900/50 px-2 py-0.5 rounded border border-neutral-200 dark:border-neutral-700">
                            <Archive className="w-3 h-3 text-neutral-400 dark:text-neutral-500" /> Archivado
                          </span>
                        )}
                        ...
                        <button
                          ...
                          className="p-1 bg-neutral-100 dark:bg-neutral-800 text-neutral-500 dark:text-neutral-400 hover:bg-danger-50 dark:hover:bg-danger-900/30 hover:text-danger-600 dark:hover:text-danger-400 rounded border border-neutral-200 dark:border-neutral-700 transition-colors shadow-3xs"
