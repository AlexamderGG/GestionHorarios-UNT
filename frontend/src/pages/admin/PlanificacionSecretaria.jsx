    ...
    <div className="animate-fade-in flex flex-col h-[calc(100vh-100px)]">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-neutral-900 flex items-center gap-2">
          <Clock className="w-6 h-6 text-primary-600" /> Planificación por Escalafón
        </h1>
        <p className="text-sm text-neutral-500 mt-1">Semestre Activo: {semestre}</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 flex-1 min-h-0">
        
        {/* PANEL IZQUIERDO: LISTADO POR ESCALAFÓN */}
        <div className="lg:col-span-4 bg-white rounded-xl shadow-sm border border-neutral-200 flex flex-col h-full overflow-hidden">
          <div className="p-4 border-b border-neutral-100 bg-neutral-50">
            <h2 className="font-bold text-neutral-800 flex items-center gap-2">
              <Users className="w-4 h-4 text-primary-600" /> Escalafón Docente
            </h2>
          </div>
            ...
            {analisis.map((doc, idx) => (
              <button
                ...
                className={`w-full text-left p-3 rounded-lg flex items-center gap-3 transition-colors ${
                  docenteSelect?.id === doc.id ? 'bg-primary-50 border border-primary-200' : 'hover:bg-neutral-50 border border-transparent'
                }`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-neutral-900 truncate">{doc.apellidos}, {doc.nombres}</p>
                  <p className="text-xs text-neutral-500 truncate">{doc.categoria} ({doc.antiguedad_anios} años)</p>
                </div>
                {doc.progreso.completado ? (
                  <CheckCircle className="w-5 h-5 text-success-500 flex-shrink-0" />
                ) : (
                  <span className="text-xs font-semibold bg-warning-100 text-warning-700 px-2 py-1 rounded-md flex-shrink-0">
        ...
        <div className="lg:col-span-8 bg-white rounded-xl shadow-sm border border-neutral-200 flex flex-col h-full overflow-hidden">
          {docenteSelect ? (
            <div className="overflow-y-auto flex-1 p-6">
              <div className="border-b border-neutral-100 pb-4 mb-6">
                <h2 className="text-xl font-extrabold text-neutral-900">{docenteSelect.apellidos}, {docenteSelect.nombres}</h2>
                <p className="text-sm text-neutral-500">Categoría: {docenteSelect.categoria} | Antigüedad: {docenteSelect.antiguedad_anios} años</p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                <div className="bg-success-50/50 border border-success-100 rounded-xl p-4">
                  <h3 className="text-sm font-bold text-success-800 mb-3 flex items-center gap-2">
                    <CheckCircle className="w-4 h-4" /> Horarios Preferidos
                  </h3>
                  <div className="flex flex-wrap gap-2">
                    {docenteSelect.disponibilidades.filter(d => d.tipo === 'PREFERIDO').map(d => (
                      <div key={d.id} className="text-xs font-medium text-success-700 bg-white border border-success-200 py-1.5 px-3 rounded-lg shadow-sm">
                    ...
                    {docenteSelect.disponibilidades.filter(d => d.tipo === 'PREFERIDO').length === 0 && (
                      <p className="text-xs text-neutral-400 italic">No registró preferencias.</p>
                    )}
                  </div>
                </div>

                <div className="bg-danger-50/50 border border-danger-100 rounded-xl p-4">
                  <h3 className="text-sm font-bold text-danger-800 mb-3 flex items-center gap-2">
                    <XCircle className="w-4 h-4" /> Restricciones (No asignar)
                  </h3>
                  <div className="flex flex-wrap gap-2">
                    {docenteSelect.disponibilidades.filter(d => d.tipo === 'RESTRINGIDO').map(d => (
                      <div key={d.id} className="text-xs font-medium text-danger-700 bg-white border border-danger-200 py-1.5 px-3 rounded-lg shadow-sm">
                        <span className="font-bold">{d.dia}:</span> {formatAMPM(d.hora_inicio)} - {formatAMPM(d.hora_fin)}
                      </div>
                    ))}
                    {docenteSelect.disponibilidades.filter(d => d.tipo === 'RESTRINGIDO').length === 0 && (
                      <p className="text-xs text-neutral-400 italic">No registró restricciones.</p>
                    )}
                  </div>
                </div>

                <div className="bg-indigo-50/50 border border-indigo-100 rounded-xl p-4 md:col-span-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {docenteSelect.horarios.map(h => (
                      <div key={h.id} className="text-xs font-medium text-indigo-700 bg-white border border-indigo-200 py-1.5 px-3 rounded-lg shadow-sm">
                        <span className="font-bold">{h.dia}:</span> {formatAMPM(h.hora_inicio)} - {formatAMPM(h.hora_fin)}
                      </div>
                    ))}
                    {docenteSelect.horarios.length === 0 && (
                      <p className="text-xs text-neutral-400 italic">No tiene clases programadas aún.</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <h3 className="text-base font-bold text-neutral-800 mb-4 flex items-center gap-2">
                <BookOpen className="w-5 h-5 text-primary-600" /> Cursos a Asignar
              </h3>
              
              <div className="space-y-3">
                {docenteSelect.asignaciones.map(asig => (
                  <div key={asig.id} className={`p-4 rounded-xl border flex items-center justify-between ${asig.programado ? 'bg-neutral-50 border-neutral-200 opacity-70' : 'bg-white border-primary-200 shadow-sm'}`}>
                    <div>
                      <p className="font-bold text-neutral-900 text-sm">[{asig.curso_codigo}] {asig.curso_nombre}</p>
                      <p className="text-xs text-neutral-500 mt-1">
                        Ciclo: <span className="font-medium text-neutral-700">{asig.curso_ciclo}</span> | 
                        Tipo: <span className="font-medium text-neutral-700">{asig.tipo}</span> | 
                        Requiere: <span className="font-medium text-neutral-700">{asig.tipo === 'Teoria' ? asig.horas_aula : asig.horas_lab} hrs seguidas</span>
                      </p>
                    </div>
                    {asig.programado ? (
                      <span className="bg-success-100 text-success-700 px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1">
                        <CheckCircle className="w-3.5 h-3.5" /> Programado
                      </span>
                    ) : (
                      <button onClick={() => handleAbrirModal(asig)} className="btn-primary py-1.5 px-4 text-xs">
                        Programar
                      </button>
                    )}
                  </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-neutral-400 p-6 text-center">
              ...
              <p className="text-lg font-medium text-neutral-600">Seleccione un Docente</p>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 animate-fade-in p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md animate-scale-in overflow-hidden">
            <div className="p-5 border-b border-neutral-100 flex justify-between items-center bg-neutral-50">
              <div>
                <h3 className="font-bold text-neutral-900">Programar: {asigModal.curso_codigo}</h3>
                <p className="text-xs text-neutral-500">{asigModal.curso_nombre} ({asigModal.tipo}) - Ciclo {asigModal.curso_ciclo}</p>
              </div>
              <button onClick={() => setAsigModal(null)} className="text-neutral-400 hover:text-neutral-600 p-1"><X className="w-5 h-5"/></button>
            </div>
            
            <div className="p-5 space-y-4">
              <div>
                <label className="block text-xs font-bold text-neutral-700 mb-1">Día</label>
                <select 
                  value={form.dia} 
              ...
              <div>
                <label className="block text-xs font-bold text-neutral-700 mb-1">
                  ...
                  {cargandoAmbientes && <span className="text-primary-600 text-[10px] ml-2 animate-pulse">(Buscando...)</span>}
                </label>
                <select 
                  value={form.ambiente_id} 
                  onChange={(e) => setForm({...form, ambiente_id: e.target.value})} 
                  className="input w-full" disabled={!form.hora_inicio || cargandoAmbientes}
                >
            ...
            <div className="p-4 border-t border-neutral-100 bg-neutral-50 flex justify-end gap-2">
              <button onClick={() => setAsigModal(null)} className="btn-ghost text-sm py-2">Cancelar</button>
              <button onClick={handleGuardarHorario} disabled={guardando || !form.ambiente_id} className="btn-primary text-sm py-2 flex gap-2">
